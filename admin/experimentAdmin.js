...
Template.tsAdminExperiments.events({
  ...
  "click .-ts-stop-experiment"() {
    ...
    return bootbox.confirm("This will end the experiment immediately. Are you sure?", function(
      res
    ) {
      if (res) {
        return Meteor.call("ts-admin-stop-experiment", expId);
      }
    });
  }
});
...
Template.tsAdminExperiments.helpers({
  numExperiments() {
    return Experiments.find().count();
  }
});

const numUsers = function() {
  return this.users != null ? this.users.length : undefined;
};

Template.tsAdminExperimentMaintenance.events({
  "click .-ts-stop-all-experiments"(e) {
    return bootbox.confirm("This will end all experiments in progress. Are you sure?", function(
      res
    ) {
      if (!res) {
        return;
...
Template.tsAdminExperimentTimeline.rendered = function() {
  ...
  const x = d3.scale.linear().range([0, $svg.width()]);

  const y = d3.scale.ordinal().rangeBands([0, $svg.height() - margin.bottom], 0.2);
  ...
  const svgX = svg.select("g.x.axis").attr("transform", `translate(0,${chartHeight})`);
  ...
  const redraw = function() {
    ...
    // Update x grid
    const grid = svgXgrid.selectAll("line.grid").data(x.ticks(10)); // More gridlines than above
  ...
  return this.autorun(() => {
    ...
    // compute new domains
    const minStart =
      d3.min(exps, e => (e != null ? e.startTime : undefined)) || TimeSync.serverTime(null, 2000);
    // a running experiment hasn't ended yet :)
    const maxEnd = d3.max(
      exps,
      e => (e != null ? e.endTime : undefined) || TimeSync.serverTime(null, 2000)
    );
...
Template.tsAdminActiveExperiments.helpers({
  experiments() {
    return Experiments.find({ endTime: { $exists: false } }, { sort: { startTime: -1 } });
  },
...
Template.tsAdminCompletedExperiments.helpers({
  experiments() {
    return Experiments.find({ endTime: { $exists: true } }, { sort: { startTime: -1 } });
  },
  duration() {
...
Template.tsAdminLogs.helpers({
  experiment() {
    return Experiments.findOne(this.instance);
  },
  logEntries() {
    return Logs.find({}, { sort: { _timestamp: -1 } });
  },
  entryData() {
    return _.omit(this, "_id", "_userId", "_groupId", "_timestamp");
  }
});
...
Template.tsAdminTreatments.helpers({
  treatments,
  zeroTreatments() {
    return Treatments.find().count() === 0;
  }
});
...
Template.tsAdminNewTreatment.events = {
  "submit form"(e, tmpl) {
    ...
    return Treatments.insert({ name }, function(e) {
      if (e) {
...
Template.tsAdminConfigureBatch.helpers({
  selectedBatch() {
    return Batches.findOne(Session.get("_tsSelectedBatchId"));
  }
});
...
Template.tsAdminBatchEditDesc.rendered = function() {
  ...
  container.editable({
    ...
    success: (response, newValue) => {
      Batches.update(this.data._id, { $set: { desc: newValue } });
...
Template.tsAdminBatchList.helpers({
  ...
  zeroBatches() {
    return Batches.find().count() === 0;
  },
  selectedClass() {
    if (Session.equals("_tsSelectedBatchId", this._id)) {
      return "info";
    } else {
      return "";
    }
