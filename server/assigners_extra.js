  ...
  setup(lookBackHours = 6) {
    ...
    const existing = Experiments.find({
      ...
      $or: [
        {
          startTime: {
            $gte: new Date(Date.now() - lookBackHours * 3600 * 1000)
          }
        },
        { startTime: null }
      ]
    ...
    this.groupConfig = TutorialRandomizedGroupAssigner.generateConfig(
      this.groupArray,
...
TurkServer.Assigners.TutorialMultiGroupAssigner = class TutorialMultiGroupAssigner extends TurkServer.Assigner {
  ...
  configure(groupArray, lookBackHours = 6) {
    ...
    this.groupConfig = TutorialMultiGroupAssigner.generateConfig(
      this.groupArray,
      this.groupTreatments
    );
    ...
    for (let i = 0; i < existing.length; i++) {
      ...
      if (count === target) {
        ...
        count > target ||
        i !== existing.length - 1 ||
        !_.isEqual(exp.treatments, this.groupConfig[i].treatments)
      ) {
        ...
        console.log(
          "Initializing multi-group assigner to group " +
            this.currentGroup +
            " (" +
            this.currentFilled +
            "/" +
  ...
  assignNext(asst) {
    ...
    if (this.currentGroup === this.groupConfig.length - 1 && this.currentInstance.isEnded()) {
      this.stopped = true;
