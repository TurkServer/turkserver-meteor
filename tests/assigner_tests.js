...
Tinytest.add(
  "assigners - tutorialGroup - assigner picks up existing instance",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialGroupAssigner(
      tutorialTreatments,
      groupTreatments
    );
...
Tinytest.add(
  "assigners - tutorialGroup - initial lobby gets tutorial",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialGroupAssigner(
      tutorialTreatments,
      groupTreatments
...
Tinytest.add(
  "assigners - tutorialGroup - autoAssign event triggers properly",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialGroupAssigner(
      tutorialTreatments,
      groupTreatments
...
Tinytest.add(
  "assigners - tutorialGroup - final send to exit survey",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialGroupAssigner(
      tutorialTreatments,
      groupTreatments
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - initial lobby gets tutorial",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialRandomizedGroupAssigner(
      tutorialTreatments,
      multiGroupTreatments,
      [8, 16, 32]
    );
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - send to exit survey",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialRandomizedGroupAssigner(
      tutorialTreatments,
      multiGroupTreatments,
    ...
    Assignments.update(asst.asstId, {
      $push: {
        instances: {
          $each: [{ id: Random.id() }, { id: Random.id() }]
        }
      }
    });
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - set up instances",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialRandomizedGroupAssigner(
      tutorialTreatments,
      multiGroupTreatments,
    ...
    created.sort(function(a, b) {
      if (a.treatments[0] === "parallel_worlds") {
        return 1;
      } else if (b.treatments[0] === "parallel_worlds") {
        return -1;
        // grab the part after "group_"
      } else {
        return parseInt(a.treatments[0].substring(6)) - parseInt(b.treatments[0].substring(6));
      }
    });
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - set up reusing existing instances",
  withCleanup(function(test) {
    const groupArr = [1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 2, 4, 4, 8, 16, 32];
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - pick up existing instances",
  withCleanup(function(test) {
    ...
    const groupConfig = TurkServer.Assigners.TutorialRandomizedGroupAssigner.generateConfig(
      groupArr,
      multiGroupTreatments
    );
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - resume with partial allocation",
  withCleanup(function(test) {
    const groupArr = [8, 16, 32];
    const assigner = new TurkServer.Assigners.TutorialRandomizedGroupAssigner(
      tutorialTreatments,
    ...
    for (let i = 0; i < groupConfig.length; i++) {
      ...
      for (
        let j = 1, end = conf.size / 2, asc = 1 <= end;
        asc ? j <= end : j >= end;
        asc ? j++ : j--
      ) {
        const asst = createAssignment();
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - resume with fully allocated groups",
  withCleanup(function(test) {
    const groupArr = [8, 16, 32];
    const assigner = new TurkServer.Assigners.TutorialRandomizedGroupAssigner(
      tutorialTreatments,
...
Tinytest.add(
  "assigners - tutorialRandomizedGroup - assign with waiting room and sequential",
  withCleanup(function(test) {
    ...
    for (exp of Array.from(exps)) {
      ...
      if (groupSize != null) {
        ...
        // Buffer group should have 8 users
        test.length(users, 8);
        totalAdded += users.length;
      }
    }
...
Tinytest.add(
  "assigners - tutorialMultiGroup - initial lobby gets tutorial",
  withCleanup(function(test) {
...
Tinytest.add(
  "assigners - tutorialMultiGroup - resumes from partial",
  withCleanup(function(test) {
    let conf, instance, j;
    let asc1, end1;
    ...
    // Generate the config that the group assigner would have
    const groupConfigMulti = TurkServer.Assigners.TutorialMultiGroupAssigner.generateConfig(
      groupArr,
      multiGroupTreatments
    );
    ...
    instance.setup();
    for (
      j = 1, end1 = filledAmount, asc1 = 1 <= end1;
      asc1 ? j <= end1 : j >= end1;
      asc1 ? j++ : j--
    ) {
      instance.addAssignment(createAssignment());
    }
...
Tinytest.add(
  "assigners - tutorialMultiGroup - resumes on group boundary",
  withCleanup(function(test) {
    let instance;
    const groupArr = [1, 1, 1, 1, 2, 2, 4, 4, 8, 16, 32, 16, 8, 4, 4, 2, 2, 1, 1, 1, 1];
...
Tinytest.add(
  "assigners - tutorialMultiGroup - send to exit survey",
  withCleanup(function(test) {
    const assigner = new TurkServer.Assigners.TutorialMultiGroupAssigner(
      tutorialTreatments,
      multiGroupTreatments,
...
Tinytest.add(
  "assigners - tutorialMultiGroup - simultaneous multiple assignment",
  withCleanup(function(test) {
    let asst;
    let i;
