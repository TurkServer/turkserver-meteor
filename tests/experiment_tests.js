...
TurkServer.initialize(function() { return setupContext = this; });
...
TurkServer.initialize(function() {
  ...
  return Meteor.defer(() => Doobie.insert({
    bar: "baz"}));
});

TurkServer.onConnect(function() { return reconnectContext = this; });
TurkServer.onDisconnect(function() { return disconnectContext = this; });
TurkServer.onIdle(function() { return idleContext = this; });
TurkServer.onActive(function() { return activeContext = this; });
...
Tinytest.add("experiment - batch - creation and retrieval", withCleanup(function(test) {
...
server code returns null`, test => test.equal(TurkServer.Assignment.currentAssignment(), null));

Tinytest.add("experiment - instance - throws error if doesn't exist", withCleanup(test => test.throws(() => TurkServer.Instance.getInstance("yabbadabbadoober")))
);

Tinytest.add("experiment - instance - create", withCleanup(function(test) {
  const treatments = [ "fooTreatment" ];

  // Create a new id to test specified ID
  const serverInstanceId = Random.id();

  const instance = batch.createInstance(treatments, {_id: serverInstanceId});
...
Tinytest.add("experiment - instance - setup context", withCleanup(function(test) {
  const treatments = [ "fooTreatment" ];
  ...
  test.isTrue(Array.from(treatment.treatments).includes("fooTreatment"),
...
Tinytest.add("experiment - instance - teardown and log", withCleanup(function(test) {
...
Tinytest.add("experiment - instance - get treatment on server", withCleanup(function(test) {
...
Tinytest.add("experiment - instance - global group", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup(); // Inserts two items

  TestUtils.sleep(100); // Let deferred insert finish

  instance.bindOperation(() => Doobie.insert({
    foo2: "bar"}));
...
Tinytest.add("experiment - assignment - reject adding user to ended instance", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - addAssignment records start time and instance id", withCleanup(function(test) {
  let needle;
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - second addAssignment does not change date", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - teardown with returned assignment", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - user disconnect and reconnect", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - user idle and re-activate", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - user disconnect while idle", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - idleness is cleared on reconnection", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - teardown while disconnected", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - teardown while idle", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - leave instance after teardown", withCleanup(function(test) {
  const instance = batch.createInstance([]);
  instance.setup();
...
Tinytest.add("experiment - assignment - teardown and join second instance", withCleanup(function(test) {
  let needle, needle1;
  const instance = batch.createInstance([]);
  instance.setup();
