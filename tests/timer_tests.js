...
Tinytest.addAsync("timers - expiration callback", withCleanup((test, next) => Partitioner.bindGroup(testGroup, function() {
  const now = new Date;
...
Tinytest.addAsync("timers - start multiple rounds", withCleanup((test, next) => Partitioner.bindGroup(testGroup, function() {
  const now = new Date;
...
Tinytest.addAsync("timers - end and start new rounds", withCleanup((test, next) => Partitioner.bindGroup(testGroup, function() {
...
Tinytest.addAsync("timers - early expiration", withCleanup((test, next) => Partitioner.bindGroup(testGroup, function() {
  const now = new Date;
...
Tinytest.addAsync("timers - robustness to multiple calls", withCleanup((test, next) => Partitioner.bindGroup(testGroup, function() {
  ...
  for (let _ = 1; _ <= 10; _++) { Meteor.defer(testFunc); }
...
Tinytest.addAsync("timers - reschedule on server restart", withCleanup((test, next) => Partitioner.bindGroup(testGroup, function() {
  const now = new Date;
